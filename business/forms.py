...
class AddServiceForm(forms.Form):
    ...
    description = forms.CharField(label='Details Of Service',max_length=30, required=True, widget=forms.Textarea)
    price_type = forms.ChoiceField(choices=price_choices)
    price = forms.DecimalField(label='Price ($)',max_digits=10, required=True)
    duration_hour = forms.ChoiceField(choices=hours_choices)
    duration_minute = forms.ChoiceField(choices=minute_choices)
    class Meta:
        model = Services
